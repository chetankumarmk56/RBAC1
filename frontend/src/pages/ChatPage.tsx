...
import { ApiError, fetchConversation, streamChat } from '../services/api'
import type { ChatMessage, ProgressStep, UserInfo } from '../types'
...
const STARTERS: Record<string, Starter[]> = {
  supervisor: [
    ...
    { text: 'Who on my team is on leave right now?', hint: 'no leave:read' },
  ],
  analyst: [
    { text: 'What is the average attendance?' },
    { text: 'Show me the performance rating breakdown.' },
    { text: 'Give me a summary report.' },
    { text: 'Show me employee salaries.', hint: 'no payroll:read' },
  ],
  hr: [
    ...
    { text: 'Show me payroll information.', hint: 'no payroll:read' },
  ],
  admin: [
    { text: 'Show me payroll information.' },
    { text: 'Show me denied access attempts.' },
    { text: 'Show me the current role permissions.' },
    { text: 'Give the analyst role access to payroll.', hint: 'no permissions:write' },
  ],
  super_admin: [
...
export default function ChatPage({
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        await streamChat(text, activeId.current, (event) => {
    ...
    [onConversationStarted, onConversationUpdated, onUnauthorized],
  )
      ...
      <div className="thread">
        {empty && (
          <div className="welcome">
            <p className="welcome-eyebrow">Signed in as {user.role.replace(/_/g, ' ')}</p>
            ...
            <details className="perms-disclosure">
              <summary>
                ...
                Your {user.permissions.length} permissions
              </summary>
              <div className="perms-grid">
                {user.permissions.map((permission) => (
                  <span key={permission} className="chip chip-mono">
                    {permission}
                  </span>
                ))}
              </div>
